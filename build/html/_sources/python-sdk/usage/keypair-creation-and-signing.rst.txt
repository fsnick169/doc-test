...
Keypairs are used to sign transactions and encrypt/decrypt messages.
They consist of a public/private key and can be generated in several
ways like by a BIP39 mnemonic:
...
By default, a keypair is using
`SR25519 <https://research.web3.foundation/en/latest/polkadot/keys/1-accounts-more.html>`__
cryptography, alternatively ED25519 and ECDSA (for Ethereum-style
addresses) can be explicitly specified:
...
This example generates a signature payload which can be signed on
another (offline) machine and later on sent to the network with the
generated signature.
...
2. Then on another (offline) machine generate the signature with given
   ``signature_payload``:
...
3. Finally on the online machine send the extrinsic with generated
   signature:
